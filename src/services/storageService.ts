...
export async function uploadAudio(file: File): Promise<string> {
    ...
    try {
        ...
        if (error || !data?.url) {
            console.error("Erro ao solicitar URL do R2 para áudio:", error || data);
            throw new Error(error?.message || "Falha ao gerar link de upload.");
        }
        ...
        if (!uploadResponse.ok) {
            console.error("Erro no upload de áudio direto para R2:", uploadResponse.statusText);
            throw new Error(`Falha no upload para o Storage: ${uploadResponse.statusText}`);
        }
        ...
        console.error("Erro geral no uploadAudio:", err);
        throw err;
    }
}
